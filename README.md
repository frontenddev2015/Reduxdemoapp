...
Store is a global object that holds application state. It allows the state to update by the dispatch action
...
function that returns javascript object.
...
Even though the action creater is ready, we still dont have idea what's the state is? for that we need to create provider

In redux all the application states exits as a single store object. what reducer do is takes state in an action and returns a new state.

reducer based upon action modify the state in pure way

connect function works by hooking up with two function with react component

mapSetsToProps
mapDispatchToProps -> needs to bind actioncreators to the applications, for this we need bindActioncreatetors
